...
import { usePublicUser } from "@/hooks/public/usePublicDoctor";
...
interface NewChatAppointmentDialogProps {
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

function NewChatAppointmentDialog({ onOpenChange, onSuccess }: NewChatAppointmentDialogProps) {
  ...
  const requestData = async () => {
    ...
    try {
      const filter = {
        ...
        excludeId: [currentProfile?.data?.body?.id],
      };
  ...
  const handleSelect = async (contact: UserResponse) => {
    try {
          ...
          return (
            ...
            >
              <Avatar className="h-9 w-9 shrink-0">
                <AvatarImage
                  src={contact?.pathAvatar ? getImageUrl(contact?.pathAvatar) : undefined}
...
export default NewChatAppointmentDialog;
