...
import { Star, User, Check, Search, Stethoscope } from "lucide-react";
...
export function StepDoctor() {
  ...
  const totalPages = publicDoctorList.data?.body?.totalPages
    ? publicDoctorList.data?.body?.totalPages - 1
    : 0;
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 px-6 rounded-2xl border border-dashed border-gray-200 bg-gray-50 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1 max-w-sm">
              {keyword
                ? `No doctors match "${keyword}". Try another keyword.`
                : "There are currently no doctors available for this specialty."}
            </p>
          </div>
        ) : (
          doctors.map((doc) => {
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {doc.degree} · {formatNumber(doc.experienceYears)} yrs exp
                      </p>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-gray-400">
                        ({formatNumber(doc.totalReviews)})
                      </span>
                    </div>

                    <div className="flex items-center gap-1 text-xs text-gray-400">
                      <User className="w-3 h-3" />
                      {formatNumber(doc.totalPatients)}
                    </div>
